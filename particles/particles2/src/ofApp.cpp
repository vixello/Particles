...
void ofApp::setup() {
	...
	gui.add(emitterRad.setup("emitterRad", 70, -100, 1000));
	...
	ofSetFrameRate(60);
	ofEnableLighting();
	cam.setPosition(428.524, 141.417, 1400);
	...
	back.load("ema-suckr-original.jpg");
	...
	if (parCount >= refresh)
	{
		...
		for (int i = 0; i < howManyAlive; i++)
		{
		   ...
		   //Particle newparticle;
			...
			switch (types) {
			case 0:
				newparticle1.emitterCenter = ofVec3f(ofGetWidth() / 2 -600, ofGetHeight() / 2+300, -370);
				break;
			case 11:
				newparticle1.emitterCenter = ofVec3f(ofGetWidth() / 2 - 100, 100, -100);
				newparticle1.lifeTime = 7.6;
				newparticle1.radius = 3;
				break;
			case 12:
				newparticle1.emitterCenter = ofVec3f(ofGetWidth() / 2 - 170, 80, -70);
...
void ofApp::draw() {
	...
	back.draw(-800, 0, -700, ofGetWidth() * 2.5, ofGetHeight() * 2);
...
void ofApp::toMesh() {
	...
	ofSetColor(21, 38, 1, 70);
	ofDrawBox(200, 0, 0, ofGetWidth() * 3, 10, ofGetWidth() * 3);
	ofDisableAlphaBlending();
	//for (auto i = 0; i < 300; i++) {
	//	for (auto j = 0; j < 300; j++) {
	//		meshGround.addVertex(ofPoint(i,0,j));
	//		meshGround.addColor(ofColor::forestGreen);
	//		meshGround.addColor(ofColor::darkGreen);
	//		meshGround.addColor(ofColor::forestGreen);
	//		meshGround.addColor(ofColor::darkGreen);
	//		meshGround.addColor(ofColor::darkGreen);
	...
	//		meshGround.addIndex(i + j * 300);
	//		meshGround.addIndex((i+2) + j * 300);
	//		meshGround.addIndex(i + (j+2) * 300);
	//		meshGround.addIndex((i + 2) + (j + 2) * 300);
	//		meshGround.addIndex(i + (j + 2) * 300);

	ofSetColor(36, 17, 0);
	ofRotateXDeg(90 );
	ofRotateZDeg(-30);
	ofDrawCylinder(400, 100, 0, 40, 350);

	ofSetColor(36, 17, 0);
	//ofRotateXDeg(-60);
	//ofRotateZDeg(-30);
	ofRotateZDeg(80);
	ofDrawCylinder(200, -350, 0, 40, 350);
	...
	//	{
	//
	//			ofVec3f position = meshGround.getVertex(count);
	//			//position.y = ofMap(ofNoise(count, ofGetElapsedTimef()), 1, 0 ,0, 5);
	//			meshGround.setVertex(count, position);
	//			count++;
